...
export default function BuyAirtime() {
  ...
  function attempt() {
    ...
    if (amount < 50) { toast.error("Minimum airtime is ₦50"); return; }
    if (wallet < amount) { toast.error("Insufficient wallet balance. Fund your wallet first."); return; }
    setPinOpen(true);
  }
      ...
      <div className="grid lg:grid-cols-[1fr_360px] gap-6">
        ...
        <aside className="space-y-4">
          ...
          <Card className="p-5 shadow-card">
            ...
            <p className="text-sm text-muted-foreground">Top up ₦5,000 or more in one purchase to unlock 3% cashback to your wallet.</p>
          </Card>
          <Card className="p-5 shadow-card">
            <h3 className="font-semibold mb-2 flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-success" /> Delivery promise</h3>
            <p className="text-sm text-muted-foreground">If your airtime doesn't arrive in 5 minutes, we refund automatically.</p>
          </Card>
        </aside>
      </div>
      <PinDialog open={pinOpen} onClose={() => setPinOpen(false)} onVerified={confirmBuy} title="Authorise airtime purchase" description={`Confirm ₦${amount.toLocaleString()} ${network.toUpperCase()} airtime to ${phone}.`} />
